...
3. **种植**：选中植物后，点击草坪上任意一行高亮的格子即可种下（全部 5 行都可以种植）。
4. **抵御僵尸**：僵尸会从右侧向左移动，进入你的草坪并攻击植物；若僵尸越过草坪左边界，游戏失败，可点击「重新开始」再来一次。
5. **通关**：清空当前关卡所有波次的僵尸后，会出现一张奖励卡片，点击收集即可解锁新植物，并进入下一关；每关最后一波僵尸来袭时，屏幕上会有醒目提示。
6. 右上角的「📖 图鉴」按钮可以随时查看所有植物 / 僵尸的图鉴介绍（会暂停游戏）。
